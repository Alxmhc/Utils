...
namespace crypt
{
	class AES
	{
		...
		public:
			...
			en(const std::vector<uint8_t> &k) : key(&k) {}
		...
		public:
			static const uint_fast8_t block_size = 16;
			de(const std::vector<uint8_t> &k) : key(&k) {}
	...
	public:
		...
		AES(const uint8_t* k, uint_fast8_t ksz) : Enc(key), Dec(key)
		{
			key.resize((ksz + 28) << 2);
	...
	const uint8_t AES::de::SboxI[256] = {
